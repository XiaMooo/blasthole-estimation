...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        #     "imgsz": eval(self.parse["imgsz"].split(" ")[-1]),
        #     "weights": self.parse["weights"].split(" ")[-1],
        #     "data" : self.parse["data"].split(" ")[-1],
        #     "nosave" : self.noSave.isChecked(),
        #     "hide_conf": not self.noConf.isChecked(),
        #     "conf" : self.confSlider.value() / 100,
        #     "iou" : self.IoUSlider.value() / 100
        # }
        self.init_slots()
        self.save_fig()
    ...
    def init_slots(self):
        ...
        self.noSave.stateChanged.connect(self.toggle_settings)
    ...
    def show_frame(self):
        ...
        if img is not None:
            ...
            with torch.no_grad():
                ...
                pred = self.model(im, augment=False, visualize=False, val=False)
                ...
                for i, det in enumerate(pred):
                    if len(det):
                        det[:, :4] = scale_coords(im.shape[2:], det[:, :4], imageSourceShape).round()
    ...
    def choose_yaml(self):
        ...
        if not self.yamlFileName:
            self.modelLabel.setText(self.lastYamlFile.split("/")[-1])
            self.parse["data"] = "--data " + self.lastYamlFile
            print("Model Chosen:", self.lastYamlFile)
        else:
            self.modelLabel.setText(self.yamlFileName.split("/")[-1])
    ...
    def toggle_settings(self):
        ...
        if self.noSave.isChecked():
            self.parse["nosave"] = "--nosave"
        else:
            self.parse["nosave"] = ""
        self.save_fig()
    ...
    def save_fig(self):
        ...
        self.opt["nosave"] = self.noSave.isChecked()
